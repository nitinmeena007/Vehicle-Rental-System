...
void Application :: renderAddNewVehicleMenu() const {
    ...
    string vehicleOptionLabel = "(1. Bike,  2. Car,  3. Truck)";
    ...
    try{
        vehicle = new Vehicle(registraionNo, VehicleType(vehicleType), seat, companyName, price, Date(pucExpirationDate));
...
void Application :: renderViewVehicleMenu() const {
    ...
    // find vehicle and display date
    gotoXY(0,3);
    try
    {
        auto vehicle = this->db->getVehicle(registraionNo);
        vehicle->display();
...
void Application :: renderAddNewUserMenu() const {
    ...
    string userContactLabel = "Enter contact no of use : ";
